...
export const CardStack = ({ items, offset, scaleFactor }: { items: Card[]; offset?: number; scaleFactor?: number }) => {
    ...
    return (
        <div className="relative h-60 w-60 md:h-60 md:w-96 md:mb-36">
            {cards.map((card, index) => {
                return (
                    <motion.div
                        key={card.id}
                        className="absolute flex w-60 flex-col justify-between rounded-3xl border border-neutral-200 bg-white p-4 shadow-xl shadow-black/[0.1] md:w-96 dark:border-white/[0.1] dark:bg-black dark:shadow-white/[0.05]"
                        style={{
